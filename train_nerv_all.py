...
from model_all import VideoDataSet, FEHNeRVDecoder, TransformInput, FEHNeRV
...
def main():
    ...
    parser.add_argument('--high_enc_strds', type=int, nargs='+', default=[], help='stride list for encoder')
    parser.add_argument('--enc_dim', type=str, default='64_16', help='enc latent dim and embedding ratio')
    parser.add_argument('--high_enc_dim', type=str, default='64_2', help='enc latent dim and embedding ratio')
...
def train(local_rank, args):
    ...
    if 'pe' in args.embed or 'le' in args.embed:
        ...
        high_total_enc_strds = np.prod(args.high_enc_strds)
        embed_hw = args.final_size / total_enc_strds**2
        enc_dim1, embed_ratio = [float(x) for x in args.enc_dim.split('_')]
        high_enc_dim1, high_embed_ratio = [float(x) for x in args.enc_dim.split('_')]
        embed_dim = int(embed_ratio * args.modelsize * 1e6 / args.full_data_length / embed_hw) if embed_ratio < 1 else int(embed_ratio)

        embed_param = float(embed_dim) / total_enc_strds**2 * args.final_size * args.full_data_length+ float(high_embed_ratio) / high_total_enc_strds**2* args.final_size * args.full_data_length
...
def evaluate(model, full_dataloader, local_rank, args, 
    ...
    if local_rank in [0, None] and quant_ckt != None:
        ...
        torch.jit.save(torch.jit.trace(FEHNeRVDecoder(model), (vid_embed[:2])), f'{args.outf}/img_decoder.pth')
        # huffman coding
        if huffman_coding:
