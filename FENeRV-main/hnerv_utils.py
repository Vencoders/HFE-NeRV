...
# from wavelets_loss import wavelet_loss,total_loss
# import focal_loss
...
def get_wav_two(in_channels, pool=True):
   ...
   #  print(filter_LL.size())
    ...
    return LL, LH, HL, HH
# class WaveletLoss(nn.Module):
#     def __init__(self, in_channels=3, pool=True, wavelet='db1', level=1):
#         super(WaveletLoss, self).__init__()
#         self.LL, self.LH, self.HL, self.HH = get_wav_two(in_channels, pool=pool)
#
#     def forward(self, predicted, target):
#         losses = []
#
#         # 对每个样本和每个通道应用小波变换
#         for i in range(predicted.shape[0]):
#             pred_LL = self.LL(predicted[i].unsqueeze(0))
#             pred_LH = self.LH(predicted[i].unsqueeze(0))
#             pred_HL = self.HL(predicted[i].unsqueeze(0))
#             pred_HH = self.HH(predicted[i].unsqueeze(0))
#
#             target_LL = self.LL(target[i].unsqueeze(0))
#             target_LH = self.LH(target[i].unsqueeze(0))
#             target_HL = self.HL(target[i].unsqueeze(0))
#             target_HH = self.HH(target[i].unsqueeze(0))
#
#             # 计算各子带的损失
#             loss_LL = torch.mean(torch.abs(pred_LL - target_LL))
#             loss_LH = torch.mean(torch.abs(pred_LH - target_LH))
#             loss_HL = torch.mean(torch.abs(pred_HL - target_HL))
#             loss_HH = torch.mean(torch.abs(pred_HH - target_HH))
#
#             # 将所有子带的损失相加
#             H_loss = (loss_LH + loss_HL + loss_HH)/3
#             sample_loss =  loss_LL+10*H_loss
...
# class WaveletLoss(nn.Module):
#     def __init__(self, in_channels=3, pool=True, alpha=1.0, log_matrix=False):
#         super(WaveletLoss, self).__init__()
#         self.LL, self.LH, self.HL, self.HH = get_wav_two(in_channels, pool=pool)
#         self.alpha = alpha
#         self.log_matrix = log_matrix
#
#     def create_weight_matrix(self, pred_subband, target_subband):
#         matrix = torch.abs(pred_subband - target_subband) ** self.alpha
#         if self.log_matrix:
#             matrix = torch.log(matrix + 1.0)
#         matrix = matrix / matrix.max()
#         matrix[torch.isnan(matrix)] = 0.0
#         return torch.clamp(matrix, min=0.0, max=1.0)
#
#     def forward(self, predicted, target):
#         losses = []
#
#         # 对每个样本和每个通道应用小波变换
#         for i in range(predicted.shape[0]):
#             # Apply wavelet transform to both predicted and target images
#             pred_LL = self.LL(predicted[i].unsqueeze(0))
#             pred_LH = self.LH(predicted[i].unsqueeze(0))
#             pred_HL = self.HL(predicted[i].unsqueeze(0))
#             pred_HH = self.HH(predicted[i].unsqueeze(0))
...
#             target_HH = self.HH(target[i].unsqueeze(0))
#
#             # Create weight matrices for each sub-band
#             weight_matrix_LL = self.create_weight_matrix(pred_LL, target_LL)
#             weight_matrix_LH = self.create_weight_matrix(pred_LH, target_LH)
#             weight_matrix_HL = self.create_weight_matrix(pred_HL, target_HL)
#             weight_matrix_HH = self.create_weight_matrix(pred_HH, target_HH)
#
#             # Calculate weighted losses for each sub-band
#             loss_LL = torch.mean(weight_matrix_LL * torch.abs(pred_LL - target_LL))
#             loss_LH = torch.mean(weight_matrix_LH * torch.abs(pred_LH - target_LH))
#             loss_HL = torch.mean(weight_matrix_HL * torch.abs(pred_HL - target_HL))
#             loss_HH = torch.mean(weight_matrix_HH * torch.abs(pred_HH - target_HH))
#
#             # Combine the losses from all sub-bands
#             sample_loss = loss_LL + loss_LH + loss_HL + loss_HH
#             losses.append(sample_loss)
#
#         # Combine the losses from all samples in the batch
#         return torch.stack(losses).squeeze()
# waveletLoss = WaveletLoss().cuda()
...
class WaveletLoss(nn.Module):
    def __init__(self, wavelet='haar', levels=1, loss_weight=1.0, reduction='mean'):
        super(WaveletLoss, self).__init__()
        self.loss_weight = loss_weight
    ...
    def forward(self, pred, target):
        ...
        # 在不需要梯度的上下文管理器中计算小波变换损失
        with torch.no_grad():
            # 计算预测图像和目标图像的小波变换
            pred_coeffs = pywt.wavedec2(pred_cpu.numpy(), self.wavelet, level=self.levels)
            target_coeffs = pywt.wavedec2(target_cpu.numpy(), self.wavelet, level=self.levels)

            # 计算小波系数之间的损失
            wavelet_loss = 0.0
            for pred_coeff, target_coeff in zip(pred_coeffs, target_coeffs):
                wavelet_loss += self.criterion(torch.tensor(pred_coeff), torch.tensor(target_coeff))

        return self.loss_weight * wavelet_loss
# vgg = VGGLoss(device='cuda')
fft = FFTLoss(loss_weight=1)
hfft = HFFTLoss(loss_weight=1)
from focal_loss import FocalFrequencyLoss as FFL
ffl = FFL(loss_weight=1.0, alpha=1.0)
# wavelet_loss = WaveletLoss(wavelet='haar', levels=3, loss_weight=1.0)
...
class PhaseLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # 计算相位损失
        phase_difference = torch.abs(pred_phase - target_phase)

        # 由于相位是周期性的，需要处理相位差大于π的情况
        # phase_difference = torch.min(phase_difference, 2 * np.pi - phase_difference)
        phase_difference = torch.abs(torch.sin(phase_difference))

        # 根据reduction参数，计算最终的损失
        if self.reduction == 'mean':
            return phase_difference.mean()
        elif self.reduction == 'sum':
            return phase_difference.sum()
        else:
...
from wavelets_loss import wavelet_guided_loss
def loss_fn(pred, target, loss_type='L2', batch_average=True):
    ...
    elif loss_type =='wavelet_loss':
        loss = wavelet_loss(pred,target)
    elif loss_type =='total_loss':
    ...
    elif loss_type == 'Fusion6_fft_dwt':
        loss = 0.7 * F.l1_loss(pred, target, reduction='none').flatten(1).mean(1) + 0.3 * (1 - ssim(pred, target, data_range=1, size_average=False))+2*fft_mse_loss(pred,target)+dwt_mse_loss(pred,target)
    elif loss_type == 'Fusion6_fft':
        loss = 0.7 * F.l1_loss(pred, target, reduction='none').flatten(1).mean(1) + 0.3 * (1 - ssim(pred, target, data_range=1, size_average=False))+fft(pred,target) #0.2:37.3,ssim:0.99  0.5:37.6,ssim:0.9903   1:37.88,ssim:0.9906
        # loss = waveletLoss(pred,target)
    elif loss_type == 'L2_test':
        loss = F.mse_loss(pred, target, reduction='none').flatten(1).mean(1)+0.1*edgeLoss(pred,target)
    elif loss_type == 'Fusion6_swt':
        loss = wavelet_guided_loss(pred,target)+F.mse_loss(pred, target, reduction='none').flatten(1).mean(1)
    ...
    elif loss_type == 'ffl_test':
        loss = ffl(pred,target)
    elif loss_type == 'Fusion10_freq':
        loss = 0.7 * F.l1_loss(pred, target, reduction='none').flatten(1).mean(1) + 0.3 * (
                    1 - ms_ssim(pred, target, data_range=1, size_average=False))
...
# def lpips_fn_single(output, gt):
#     lpips = lploss_fn.forward(output.detach(),gt.detach())
#     return lpips.cpu()
